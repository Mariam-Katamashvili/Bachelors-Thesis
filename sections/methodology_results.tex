...
The Kleene star is a restricted, tail-recursive form of iteration, and this
restriction is what keeps it within the regular languages. Writing
...
states here are canonical representatives modulo $\sim$. Ergo only finitely many
distinct states are ever discovered, and the process stops as soon as no
new canonical form appears. In a constructive setting ``finite'' is not a
single notion, and the distinction matters in Rocq. The derivative set is finite up to
the equivalence $\sim$, not up to syntactic equality. The Coquand--Siles result gives
a covering list, so the set is Kuratowski-finite. That alone does not bound the number
of distinct states, since the checker cannot count entries it cannot tell apart. One
first needs decidable equality on canonical forms modulo $\sim$. This is exactly the
decision procedure for regular expression equivalence in type theory of Coquand and
Siles~\cite{CoquandRegexEquality}. With it the covering list collapses to a bijection
with a prefix, so the set is inductively finite, and the number of canonical
derivatives becomes an explicit decidable bound. That bound is what the termination
argument uses.
\end{remark}
...
does not need. Rocq requires it. Rocq accepts a recursive function only when it can see
that the function stops, and it sees this by checking that each call works on a smaller
piece of some input. The exploration does not fit this pattern. \texttt{goto} calls itself
on the new derivative \texttt{q\_a}, which is not a smaller piece of anything it was given.
We know it stops, because there are only finitely many states, but Rocq cannot see that
on its own. The counter makes it visible. \texttt{goto} starts with some \texttt{fuel} and
spends one unit each time it reaches a new state, and it stops when the fuel runs out. The
fuel only goes down, so Rocq can tell the function stops.

The amount of fuel does not affect correctness. When a transition is missing from the
table, \texttt{step} computes $[\mathcal{D}_a(q)]$ directly instead, so
\texttt{dfa\_correctness} holds for \emph{every} value of \texttt{fuel}. Too little fuel
never gives a wrong language. It only leaves the table partial, and the missing
transitions are computed on demand.

It is enough to give \texttt{goto} as much fuel as there are reachable states. The fuel
bounds how deep the search goes, not how many states it finds in all. Each step deeper
reaches a state not seen before, so the deepest chain is no longer than the number of
states. One unit is
spent per new state, and the states found along one path are all different, so no path is
longer than the number of states. That number is finite, so enough fuel always exists, and
more fuel does no harm. In practice one picks an upper bound on the number of states and
raises it until the set of discovered states stops growing.
\end{remark}
