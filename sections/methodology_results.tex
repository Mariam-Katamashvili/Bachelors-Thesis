...
This work proceeds in the style of theoretical computer science backed by formal
verification: every notion is introduced by a precise definition, every claim is
stated as a lemma or theorem with a proof, and a large part of the development is
also mechanised in the Rocq proof assistant~\cite{Rocq}, so that the machine checks
what the text argues by hand. Because the formalisation is central to the thesis, it
is presented next to the mathematics rather than in a section of its own, and each
mechanised definition or theorem is followed by the corresponding code quoted
directly from the development~\cite{OurRocqRepo}. The presentation moves in two
layers, first the crisp theory of derivatives and the derivative automaton, then the
fuzzy layer that replaces exact symbol matching by cut-similarity. Throughout,
$\Sigma$ denotes a finite, nonempty alphabet, $\varepsilon$ the empty word and
$\Sigma^{*}$ the set of all words over $\Sigma$, with $r,s,t$ ranging over regular
expressions, $a,b$ over symbols, $u,v,w$ over words and $L,K$ over languages.

\subsection{Preliminaries}\label{sec:prelim}

The basic objects of the theory are the following.
...
\mathcal{L}(r \cdot s)   &= \mathcal{L}(r)\cdot\mathcal{L}(s), \\
...
$\{a\}$; both use $\neg$ or $\&$ and are therefore regular but not restricted.
...
boolean connectives, and concatenation and Kleene star are decided by a bounded
...
\begin{lstlisting}[style=rocqstyle,caption={Nullability and its correctness (\texttt{Nullable.v}).},label={lst:nullable}]
...
\medskip
Semantic equivalence is the relation that ultimately matters, but it is not
finitely axiomatisable~\cite{Salomaa,Kozen} and is awkward to compute with
directly. Following Brzozowski~\cite{B62} and Coquand and
...
following equational laws. For choice it includes idempotence, commutativity and
...
is built from \emph{smart constructors} that bake the laws above into the way
...
\begin{proof}
This is the mechanised theorem \texttt{similar\_iff\_nf\_eq} of
...
\begin{lstlisting}[style=rocqstyle,caption={Canonicalisation/normalisation, mechanised (\texttt{CanonicalizationCorrectness.v}).},label={lst:canoncorrect}]
...
\begin{proof}
This is the mechanised theorem \texttt{similar\_sound} of
...
\begin{lstlisting}[style=rocqstyle,caption={Soundness of normalisation, mechanised (\texttt{CanonicalizationCorrectness.v}).},label={lst:soundcorrect}]
...
\begin{remark}[Substitution and the boundary of regularity]\label{rem:substitution}
The Kleene star is a restricted, tail-recursive form of iteration, and it is
exactly this restriction that keeps it within the regular languages. It is
instructive to see what happens when the restriction is lifted. Writing
$r[a]$ to highlight an occurrence of a symbol $a$ in $r$, and $r[s]$ for the
expression obtained by substituting $s$ for that occurrence, one may define a
general iteration $r[a]^{\#}$ by
$\mathcal{L}(r[a]^{\#})=\{\varepsilon\}\cup\bigcup_{n\ge0}\mathcal{L}(r[a]^{\#}_{n})$,
where $\mathcal{L}(r[a]^{\#}_{0})=\mathcal{L}(r[a])$ and
$\mathcal{L}(r[a]^{\#}_{n+1})=\mathcal{L}(r[\,r[a]^{\#}_{n}\,])$. If the
substitution is not tail-recursive this escapes the regular languages: for
$r=a[a]b$ one obtains
$\mathcal{L}(r[a]^{\#})=\{\varepsilon\}\cup\{a^{n+1}b^{n}\mid n\in\mathbb{N}\}$,
which is not regular. This delimits the setting of the thesis, which stays with the
regular operators, for which derivatives are finite and the automaton construction
terminates.
\end{remark}
...
\[
\begin{array}{rcl}
    \mathcal{D}_a(\emptyset) &=& \emptyset,\\[0.4ex]
    \mathcal{D}_a(\varepsilon) &=& \emptyset,\\[0.4ex]
    \mathcal{D}_a(a) &=& \varepsilon,\\[0.4ex]
    \mathcal{D}_a(b) &=& \emptyset \quad (a\neq b),\\[0.4ex]
    \mathcal{D}_a(r+s) &=& \mathcal{D}_a(r)+\mathcal{D}_a(s),\\[0.4ex]
    \mathcal{D}_a(r\cdot s) &=& \mathcal{D}_a(r)\cdot s + \nu(r)\cdot \mathcal{D}_a(s),\\[0.4ex]
...
This is the heart of the development. The symbol derivative $\mathcal{D}_a(\cdot)$
is the function \texttt{D\_char}; note how the \texttt{Seq} clause reproduces
...
$a^{*}$ by a power of $a$ is again $a^{*}$ up to similarity, which is exactly why
...
\begin{proof}
The correspondence is established by machine in Rocq, as the lemma
\texttt{D\_correct} shown in Listing~\ref{lst:dcorrect}. It states that a word $w$
lies in the derivative \texttt{D u r} exactly when \texttt{u ++ w} lies in $r$,
which is the membership form of the equality
$\mathcal{L}(\mathcal{D}_u(r))=\partial_u(\mathcal{L}(r))$. Its single-symbol core
is \texttt{D\_char\_correct}, proved by structural induction on the expression with
the \texttt{Seq} and \texttt{Star} cases doing the real work, and \texttt{D\_correct}
follows from it by a short induction on the word.
\end{proof}

\begin{lstlisting}[style=rocqstyle,caption={The correspondence theorem (\texttt{Derivatives.v}).},label={lst:dcorrect}]
...
some expression, that is, when \texttt{exists r, L =i den r}. If $r$ witnesses $L$,
then Theorem~\ref{the:two-derivative-mapping}, in the form \texttt{D\_correct},
makes $\mathcal{D}_u(r)$ a witness for $\partial_u(L)$, because
$\partial_u(L)=\mathcal{L}(\mathcal{D}_u(r))$; hence $\partial_u(L)$ is again
denoted by an expression and is regular.
\end{proof}

\begin{lstlisting}[style=rocqstyle,caption={Regularity of derivatives, mechanised (\texttt{Derivatives.v}).},label={lst:regular}]
...
\begin{theorem}[Finiteness of derivatives]\label{the:derivative-set-is-finite}
...
many possible languages for each term and hence finitely many possible unions.
...
is finite, and hence so is $\{\partial_u(L)\mid u\in\Sigma^{*}\}$.
\end{proof}

\subsection{Finite Automata}\label{sec:dfa}

The finiteness result of the previous subsection is exactly what makes it possible
to turn a regular expression into a finite automaton: the canonical
derivatives become the states and differentiation the transition function. Each state
denotes the language that remains after some prefix of the input has been consumed,
and the transitions are one-step derivatives, computed on canonical representatives
so that the number of states stays finite.
...
$[r]:=\mathsf{nf}(r)$. The \emph{derivative DFA} of a regular expression $r$ is the
five-tuple $M=\langle Q,\Sigma,\delta,q_0,F\rangle$ whose components are the
following.
\begin{itemize}
  \item $Q=\{\,[\,\mathcal{D}_u(r)\,]\mid u\in\Sigma^{*}\,\}$ is the set of states.
        Each state is the canonical form $[t]=\mathsf{nf}(t)$ of a derivative of
        $r$, so that two derivatives denoting similar expressions are the same
        state.
  \item $\Sigma$ is the input alphabet, the same finite alphabet over which $r$ is
        written.
  \item $\delta:Q\times\Sigma\to Q$ is the transition function, obtained by taking
        a one-step derivative and canonicalising it,
        \[
          \delta(q,a)=[\,\mathcal{D}_a(q)\,].
        \]
  \item $q_0=[\,r\,]\in Q$ is the initial state, namely the canonical form of $r$
        itself, which is consistent with $\mathcal{D}_\varepsilon(r)=r$.
  \item $F\subseteq Q$ is the set of accepting states, those whose language
        contains the empty word,
        \[
          F=\{\,q\in Q\mid \nu(q)=\varepsilon\,\}.
        \]
...
Because every state is stored in canonical form, two derivatives are recognised as
the same state by syntactic equality of their normal forms, not by comparing the
languages they denote. The algorithm below builds the automaton: it keeps taking
derivatives and adds a new state each time it meets a canonical form it has not
seen before.

\begin{lstlisting}[style=thesiscode,label={lst:deriv-dfa},caption={Derivative-based DFA construction.}]
goto(q, a, Q, delta):
    q_a := [ D(a, q) ]
    if q_a in Q then
        delta[(q, a)] := q_a
...
Since $\Sigma$ is finite, the exploration branches finitely at each state.
Moreover, Coquand and Siles~\cite{CoquandRegexEquality} show that the set of
Brzozowski derivatives of any expression is finite with respect to $\sim$, and the
states here are canonical representatives modulo $\sim$; hence only finitely many
distinct states are ever discovered and the algorithm terminates.
...
\begin{lstlisting}[style=rocqstyle,caption={The run of the automaton and the mechanised correctness proof (\texttt{Automata.v}).},label={lst:dfa}]
...
Note that states are identified only up to canonical form, not up to full semantic
equivalence. Two semantically equivalent expressions can therefore end up as
distinct states: for instance $a^{*}$ and $(\varepsilon+a)^{*}$ denote the same
language but have different canonical forms, so the construction keeps them apart.

\medskip
\noindent\textbf{Example.}
Let $\Sigma=\{a,b\}$ and run the construction on the restricted regular expression
$r=(a+b)^{*}\cdot a$. The algorithm of Listing~\ref{lst:deriv-dfa} starts from the
...
Differentiating $q_0$ by $b$ instead,
\[
\begin{aligned}
\mathcal{D}_b\bigl((a+b)^{*}\cdot a\bigr)
  &= \mathcal{D}_b\bigl((a+b)^{*}\bigr)\cdot a
     + \nu\bigl((a+b)^{*}\bigr)\cdot \mathcal{D}_b(a)\\
  &= \bigl(\mathcal{D}_b(a+b)\cdot (a+b)^{*}\bigr)\cdot a
     + \varepsilon\cdot \mathcal{D}_b(a)\\
  &= \bigl((\emptyset+\varepsilon)\cdot (a+b)^{*}\bigr)\cdot a
     + \varepsilon\cdot\emptyset,
\end{aligned}
\]
where $\mathcal{D}_b(a+b)=\emptyset+\varepsilon$ and $\mathcal{D}_b(a)=\emptyset$.
Canonicalising with $\emptyset+\varepsilon\sim\varepsilon$,
$\varepsilon\cdot(a+b)^{*}\sim(a+b)^{*}$, $\varepsilon\cdot\emptyset\sim\emptyset$ and
$s+\emptyset\sim s$ gives $\mathcal{D}_b(r)\sim r$, so
$\delta(q_0,b)=[\mathcal{D}_b(r)]=[r]=q_0$, an already-known state.

The only state still to process is $q_1=[(a+b)^{*}\cdot a + \varepsilon]$. By the sum
...
$\nu(q_1)=\nu((a+b)^{*}\cdot a)+\nu(\varepsilon)=\emptyset+\varepsilon=\varepsilon$,
so $q_1$ is accepting. The construction therefore returns the two-state automaton
...
The effect of canonicalisation on the size of the automaton is seen most clearly on
a tiny instance.
...
With canonicalisation, the start state is $q_0=[a^{*}]=a^{*}$, and its one-step
derivative on $a$ is
\[
  \mathcal{D}_a(a^{*})=\mathcal{D}_a(a)\cdot a^{*}=\varepsilon\cdot a^{*}\;\sim\;a^{*},
\]
so $\delta(q_0,a)=[\mathcal{D}_a(a^{*})]=[a^{*}]=q_0$. The canonical derivative is the
start state again, the worklist empties immediately, and the construction returns the
one-state automaton of Figure~\ref{fig:canon-yes}: a single accepting state with a
self-loop on $a$, recognising $\mathcal{L}(a^{*})$.

Without canonicalisation, states are compared as raw syntax. Reading $a$ once gives
the derivative $s_1=\varepsilon\cdot a^{*}$, which denotes the same language as
$a^{*}$ and is similar to it, but is a different expression, so it becomes a new
state. Reading a further $a$ gives
\[
  \mathcal{D}_a(s_1)
  = \mathcal{D}_a(\varepsilon)\cdot a^{*} + \nu(\varepsilon)\cdot\mathcal{D}_a(a^{*})
  = \emptyset\cdot a^{*} + \varepsilon\cdot(\varepsilon\cdot a^{*})
  =: s_2,
\]
larger still and again distinct, then $s_3=\mathcal{D}_a(s_2)$ larger again, and so on.
Every $s_i$ denotes $a^{*}$ and every pair is similar, yet no two are syntactically
equal, so the construction keeps inventing states and never terminates, as suggested
by Figure~\ref{fig:canon-no}. Canonicalisation is exactly what collapses the whole
chain $a^{*},\,\varepsilon\cdot a^{*},\,s_2,\,\ldots$ to the single representative
$a^{*}$, turning the unbounded chain of Figure~\ref{fig:canon-no} into the one-state
automaton of Figure~\ref{fig:canon-yes}.
...
\end{tikzpicture}
\caption{With canonicalisation: the derivative automaton of $a^{*}$ over
$\Sigma=\{a\}$ has the single accepting state $q_0=[a^{*}]$ with a self-loop on $a$.}
\label{fig:canon-yes}
\end{figure}
...
\begin{tikzpicture}[->,>={Stealth[round]},shorten >=1pt,auto,semithick,
  ...
  \path (q0) edge node {$a$} (s1)
        ...
        (s3) edge node {$a$} (dots);
\end{tikzpicture}
\caption{Without canonicalisation: reading $a$ repeatedly produces an unbounded chain
of distinct raw derivatives $a^{*},\ s_1=\varepsilon\cdot a^{*},\ s_2,\ \ldots$, all
denoting $a^{*}$ and pairwise similar but never syntactically equal, so the
...
\noindent\textbf{Example.} Three standard T-norms are Gödel's minimum
$x\otimes y=\min(x,y)$, the product $x\otimes y=x\cdot y$, and the {\L}ukasiewicz
T-norm $x\otimes y=\max(0,x+y-1)$. Each satisfies the four axioms, for instance
$\min(x,1)=x$, $x\cdot 1=x$ and $\max(0,x+1-1)=x$ realise the neutral element. Only
Gödel's minimum is idempotent ($\min(x,x)=x$ for every $x$), a property that will
matter when cuts are formed below; this is the T-norm the thesis works with.

\medskip
The development formalises a T-norm as a record \texttt{T\_norm} bundling exactly
the axioms above, together with closedness on $[0,1]$, and provides Gödel's minimum
\texttt{smin} as the instance \texttt{godel\_t\_norm}, whose record fields are
discharged by the corresponding lemmas about \texttt{smin}.
...
of $\otimes$. These notions are exactly those used in similarity-based approximate
...
In Rocq this is the recursive function \texttt{Rw\_T}, parameterised by the T-norm
\texttt{T}: it is $1$ on two empty words, $0$ when exactly one is empty, and
otherwise the T-norm combination of the head similarity and the recursively
computed tail similarity. With Gödel's minimum this is the minimum recursion.
...
In Rocq this is \texttt{RL\_T}, the minimum of the two directions of an infimum of
suprema of word similarities; the supremum and infimum of a function over a
language are \texttt{SupL} and \texttt{InfL}, each taking a proof that the family is
bounded. The outer combination is the minimum, while the inner word similarity
\texttt{Rw\_T T} carries the chosen T-norm.
...
Viewing words as regular expressions, Definition~\ref{def:Rw} extends naturally to
...
Fixpoint GE (r s : regex) : simval :=
  ...
  | RS.And s1 s2 =>
      match r with
      | RS.And r1 r2 => smin (GE r1 s1) (GE r2 s2)
      | _ => 0
      end
  | RS.Neg s1 =>
      match r with
...
\end{cases}
\]
where $p\in[0,1]$. Definition~\ref{def:Rexp-godel} therefore has to be
...
The clause for $(\ )^{*}$ is rather strict for a general T-norm, but no definition
...
\texttt{T}: the choice and concatenation clauses both combine the subexpression
values with \texttt{tnorm T}, the star clause uses \texttt{tnorm\_power\_inf T},
the infimum of the T-norm powers $\inf_n(\bigotimes^{n}x)$, and intersection and
complement are sent to $0$ since the fragment is restricted. Under Gödel's minimum,
...
\begin{enumerate}
\item $\mathcal{R}^{e}$ is a similarity relation on $E^{\Sigma}$, and hence on
...
\begin{proof}
All three are mechanised, and bundled together in the single lemma
\texttt{general\_\allowbreak similarity\_\allowbreak relations} of
Listing~\ref{lst:simproofs}: for any
T-norm \texttt{T} that is left-continuous and with respect to which the base
relation \texttt{R} is transitive, $\mathcal{R}^{e}$ (as \texttt{RE\_T}),
$\mathcal{R}^{w}$ (as \texttt{Rw\_T}) and $\mathcal{R}^{L}$ (as \texttt{RL\_T}) are
all $T$-similarity relations. The individual proofs \texttt{RE\_T\_is\_similarity},
\texttt{Rw\_T\_is\_similarity} and \texttt{RL\_T\_is\_similarity} go by induction: on
the length of the words for $\mathcal{R}^{w}$ (the transitivity step combines the
head and tail bounds through monotonicity of the T-norm), on the common shape of
the two expressions for $\mathcal{R}^{e}$, and, for $\mathcal{R}^{L}$, from the
word-level result together with the least-upper-bound and greatest-lower-bound
properties of \texttt{SupL} and \texttt{InfL}.
\end{proof}

\begin{lstlisting}[style=rocqstyle,caption={The three similarity relations, mechanised (\texttt{SimilarityRelations.v}).},label={lst:simproofs}]
Definition restricted_regex_type :=
  { r : regex | RS.restricted_regex r }.

Definition RE_T_restricted
  (r s : restricted_regex_type) : simval :=
  RE_T (proj1_sig r) (proj1_sig s).
...
The syntactic similarity is, in general, only a lower bound for the semantic one.

\begin{theorem}[Syntax--semantics inequality]\label{thm:Rr-RL-bridge}
For all restricted regular expressions $r_1$ and $r_2$,
\[
  \mathcal{R}^{e}(r_1,r_2)\le\mathcal{R}^{L}\bigl(\mathcal{L}(r_1),\mathcal{L}(r_2)\bigr).
\]
\end{theorem}

\begin{proof}
Fix words $w_1\in\mathcal{L}(r_1)$ and $w_2\in\mathcal{L}(r_2)$ obtained by reading
the same syntactic positions of $r_1$ and $r_2$. Comparing position by position,
$\mathcal{R}^{e}(r_1,r_2)\le\mathcal{R}^{w}(w_1,w_2)$, because on a matching shape the
value of $\mathcal{R}^{e}$ is the T-norm combination of the symbol similarities,
which is exactly what $\mathcal{R}^{w}$ computes for the corresponding words. Now
$\mathcal{R}^{L}(\mathcal{L}(r_1),\mathcal{L}(r_2))$ is the minimum of two infima of
suprema of word similarities; in each direction the inner supremum over the opposite
language is at least $\mathcal{R}^{w}(w_1,w_2)$, so the outer infimum is bounded below
by $\mathcal{R}^{e}(r_1,r_2)$. Hence
$\mathcal{R}^{e}(r_1,r_2)\le\mathcal{R}^{L}(\mathcal{L}(r_1),\mathcal{L}(r_2))$.
\end{proof}
...
\item Theorem~\ref{thm:Rr-RL-bridge} fails once $\&$ and $\neg$ are allowed. If
...
Finally, a finiteness property of syntactic similarity is what keeps the fuzzy
derivative finite.
...
\begin{proof}
This is mechanised as the lemma \texttt{finite\_syntactic\_similarity} of
Listing~\ref{lst:finite}. The development builds an explicit finite list
...
\begin{lstlisting}[style=rocqstyle,caption={Finiteness of syntactic similarity, mechanised (\texttt{SimilarityRelations.v}).},label={lst:finite}]
...
The derivative now lifts to the fuzzy setting. Comparing only the consumed
...
cut, so the whole word is matched. The similarity infrastructure of
Section~\ref{sec:fuzzy-relations} is mechanised in Rocq, but the fuzzy derivative
and the fuzzy automaton developed in this section are so far established on paper;
carrying them into Rocq on top of that infrastructure is ongoing research and is
taken up again as future work in Section~\ref{sec:conclusion}.
...
Both expression derivatives are purely syntactic: by
...
\begin{proof}
Each of $\mathcal{R},\mathcal{R}^{w},\mathcal{R}^{e},\mathcal{R}^{L}$ is a similarity
relation (Lemma~\ref{lem:sim-relations}): reflexive with diagonal value $1$,
symmetric, and transitive with respect to the minimum. Reflexivity gives the
diagonal value $1\ge\mu$, so each cut relation is reflexive; symmetry of the
underlying relation gives symmetry of the cut; and for transitivity, if
$\mathcal{R}(x,y)\ge\mu$ and $\mathcal{R}(y,z)\ge\mu$ then, by the reverse triangle
inequality, $\mathcal{R}(x,z)\ge\min(\mathcal{R}(x,y),\mathcal{R}(y,z))\ge\mu$. The
same three steps apply verbatim to $\mathcal{R}^{w}$, $\mathcal{R}^{e}$ and
$\mathcal{R}^{L}$. The letterwise characterisation follows from
$\mathcal{R}^{w}(a_1\cdots a_n,b_1\cdots b_m)=\min_{1\le i\le n}\mathcal{R}(a_i,b_i)$
when $n=m$, and $0$ otherwise: the minimum is at least $\mu$ exactly when every
factor is.
\end{proof}
...
The next theorem is the key structural fact: once letters are identified up to
...
$q_\mu^{*}(v)=q_\mu^{*}(w)$, i.e.\ $w\sim_\mu^{\Sigma^{*}} v$; hence
...
$b,d,b$, each of which is $\mu$-similar exactly to the words in $\{b,d\}$. Hence
\[
...
of a regular language is regular. Hence $\partial_a^{\mu}(\mathcal{L})$ is regular.
...
$\mu$-similarity of the suffix therefore yields precisely the set described in
...
the inverse image of an element of the finite set $D^{\mu}$. Hence
...
derivatives, terminates. The resulting automaton recognises the words that lie
...
$\mathcal{L}_\Sigma^{\mu}(r)=(q_\mu^{*})^{-1}\bigl(q_\mu^{*}(\mathcal{L}(r))\bigr)$ for
the words that are $\mu$-similar to some word of $r$, the automaton satisfies
...
form of the fuzzy derivative. Hence $\mathcal{L}(M^{\mu})=\mathcal{L}_\Sigma^{\mu}(r)$.
\end{proof}

Equivalently, by Theorem~\ref{the:quotient-fuzzy-derivative} this automaton is the
derivative automaton of the quotient language $\mathcal{L}(r)^{\mu}$ over the finite
alphabet $\Sigma^{\mu}$, read back along $q_\mu^{*}$: on the quotient alphabet the
fuzzy automaton is an ordinary derivative automaton, and it accepts a word precisely
when that word is $\mu$-similar to some word denoted by $r$.
