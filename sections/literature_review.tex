...
developed in parallel. It is worth seeing where each of them stands before
...
expressions denote the same language and with axiomatising that equivalence. That
is a different question from how to match efficiently, but a closely related one,
...
expressions~\cite{B64}. He showed that differentiating an expression with respect to
a symbol, together with a test for whether the empty word is accepted, is enough
both to decide membership and to construct a deterministic automaton, provided
expressions are identified up to a few simplifications so that only finitely many
derivatives arise. Owens, Reppy and Turon re-examined the construction from a modern
implementation perspective~\cite{OwensReppyTuron}. They showed that it remains both
...
where paper proofs tend to be informal. It is also the foundation on which the
...
mostly grown from a different root. Most of this work gives automata and
expressions membership values rather than crisp acceptance, so a word is
...
monoids~\cite{LP05}, Stamenkovi\'c and \'Ciri\'c gave constructions of fuzzy automata
...
Alves, Kesner and Ramos~\cite{AlvesKesnerRamos2025}. The technical background for all
of this is the theory of triangular norms and fuzzy logic, of which Metcalfe's notes
give a brief account~\cite{met05}.
...
crisp and instead loosens the matching of symbols. A symbol then matches any other
...
present work therefore connects the derivative-based, formally verified theory of
regular expressions with similarity-based approximate matching.
